...
const ProductCard = ({ product }) => {
  ...
  const isFavorited = useSelector((state) =>
    selectIsFavorite(state, product._id || product.id)
  )
...
const ProductListPage = () => {
  ...
  useEffect(() => {
    if (!hasFetchedProducts.current && allProducts.length === 0) {
  ...
  const filteredByPrice = products.filter((product) => {
    ...
    if (priceRange === 'all') return true
    if (priceRange === 'under-10k') return price < 10000
    if (priceRange === '10k-20k') return price >= 10000 && price < 20000
    if (priceRange === '20k-50k') return price >= 20000 && price < 50000
    if (priceRange === 'above-50k') return price >= 50000
        ...
        >
          <div className="text-center">
            ...
            <p className="text-sm text-purple-400/60">
              Fetching data from MongoDB via Backend API
            </p>
          </div>
        </main>
      </>
  ...
  return (
    <>
      ...
      <main
        className="min-h-screen text-purple-400 pt-24 pb-12"
        style={{ background: '#09070f' }}
      >
        <style>{`
          @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800&display=swap');
          ...
          {filteredByPrice.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
              ...
              <div>
                ...
                <p className="text-purple-100/60 text-sm max-w-md leading-relaxed">
                  No matches were found for your search or filters. Try a
                  different keyword, category, or price range.
                </p>
              </div>
