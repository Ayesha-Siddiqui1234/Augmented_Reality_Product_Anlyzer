...
const CartItemWith3D = ({ item, onIncrease, onDecrease, onRemove }) => {
  ...
  return (
    <div className="card-glow rounded-2xl border border-purple-400/15 bg-[#09070f]/60 backdrop-blur-md p-6 flex gap-6">
      {/* 3D Model Preview */}
      {product?.arSupported && product?.modelUrl ? (
        <div
          onClick={() => navigate(`/products/${product.slug}/3d-view`)}
          className="flex-shrink-0 w-40 h-40 rounded-xl overflow-hidden bg-gradient-to-br from-purple-400/10 to-purple-600/10 border border-purple-400/20 cursor-pointer group relative"
        >
          <model-viewer
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center">
            <span className="opacity-0 group-hover:opacity-100 transition-opacity text-white text-xs font-bold bg-purple-400/90 px-3 py-1.5 rounded-full">
              🧊 View in 3D
            </span>
          </div>
        </div>
        ...
        <div
          ...
          className="flex-shrink-0 w-40 h-40 rounded-xl overflow-hidden bg-purple-400/5 cursor-pointer group"
        >
          <img
            src={item.imageUrl}
            alt={item.name}
      ...
      <div className="flex-1 flex flex-col justify-between">
        <div>
          <h3
            onClick={() => navigate(`/products/${item.productId}`)}
            className="text-purple-100 font-bold text-lg mb-2 cursor-pointer hover:text-purple-400 transition"
          >
            {item.name}
          </h3>
          ...
          {product?.arSupported && (
            <span className="inline-block mt-2 px-3 py-1 text-[10px] font-bold tracking-widest rounded-full bg-emerald-400/20 text-emerald-300 border border-emerald-400/30 uppercase">
              AR Ready
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 mt-4">
          <div className="flex items-center gap-3 bg-purple-400/5 border border-purple-400/20 rounded-xl px-4 py-2">
            <button
              onClick={() => onDecrease(item.productId)}
              className="w-8 h-8 rounded-lg bg-purple-400/10 text-purple-400 font-bold hover:bg-purple-400/20 transition flex items-center justify-center"
            >
              −
            </button>
            <span className="w-12 text-center text-lg font-bold text-purple-400">
              {item.quantity}
            </span>
            <button
              onClick={() => {
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl border border-red-400/30 bg-red-400/5 text-red-400 text-sm font-semibold hover:bg-red-400/10 transition"
          >
            Remove
          </button>
        </div>
      ...
      <div className="flex-shrink-0 text-right">
        <p className="text-purple-400/60 text-xs uppercase tracking-widest mb-2">Subtotal</p>
...
const CartPage = () => {
  ...
  const handleIncrease = (productId) => {
    ...
    if (cartItem) {
      dispatch(increaseQuantity({ userId, productId, stock: cartItem.stock }))
    }
  }
      ...
      <main className="min-h-screen text-purple-400" style={{background:'#09070f'}}>
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="flex items-center justify-between mb-12 flex-wrap gap-4">
            <div>
              <p className="text-purple-400/60 text-xs tracking-[0.4em] uppercase mb-3">Your Shopping</p>
          ...
          {cartItems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
              ...
              <div>
                <h2 className="text-2xl font-bold text-purple-400 mb-2">Your cart is empty</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-1">
                <div className="sticky top-32 card-glow rounded-2xl border border-purple-400/15 bg-[#09070f]/60 backdrop-blur-md p-6">
                  <h2 className="text-2xl font-bold text-purple-400 mb-6">Order Summary</h2>
                  
                  <div className="space-y-4 mb-6">
                    <div className="flex justify-between text-purple-100/60">
                      <span>Subtotal</span>
                    ...
                    <div className="border-t border-purple-400/20 pt-4">
                      <div className="flex justify-between text-purple-400 text-xl font-bold">
                        <span>Total</span>
                  ...
                  <button
                    ...
                    className="w-full py-4 rounded-full bg-purple-400 text-black font-bold text-sm hover:bg-purple-300 transition-all duration-300 shadow-[0_0_40px_rgba(153,85,255,0.3)] mb-4"
                  >
                    Proceed to Checkout
                  </button>
                  ...
                  <button
                    onClick={() => navigate('/products')}
                    className="w-full py-3 rounded-full border border-purple-400/30 bg-purple-400/5 text-purple-400 font-semibold text-sm hover:bg-purple-400/10 transition-all"
                  >
                    Continue Shopping
                  </button>
                  ...
                  <div className="mt-6 pt-6 border-t border-purple-400/10 space-y-3">
                    <div className="flex items-center gap-3 text-sm text-purple-100/60">
