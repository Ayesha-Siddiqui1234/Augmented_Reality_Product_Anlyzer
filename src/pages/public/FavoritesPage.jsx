...
const normalizeFavoriteProduct = (favoriteItem) => {
  const product =
  ...
  const imageUrl =
    ...
    favoriteItem?.images?.[0] ||
    ''

  const price = Number(product.price || favoriteItem?.price || 0)

  const originalPrice = Number(
    product.originalPrice ||
      favoriteItem?.originalPrice ||
      product.price ||
      favoriteItem?.price ||
      0
  )
  ...
  return {
    ...
    id,
    name,
    slug: product.slug || favoriteItem?.slug || makeSlug(name),
    imageUrl,
    images: product.images || favoriteItem?.images || (imageUrl ? [imageUrl] : []),
    category: product.category || favoriteItem?.category || 'Product',
    categoryLabel:
    ...
    price,
    originalPrice,
    isNew: product.isNew || favoriteItem?.isNew || false,
    arSupported:
      product.arSupported ||
      product.arEnabled ||
      favoriteItem?.arSupported ||
      favoriteItem?.arEnabled ||
      Boolean(product.modelUrl || product.glbModel),
  }
}
...
const FavoritesPage = () => {
  ...
  const isAuthenticated = useSelector(selectIsAuthenticated)
  const currentUser = useSelector(selectCurrentUser)
  const rawFavoriteProducts = useSelector(selectFavoriteProducts)
  const loading = useSelector(selectFavoritesLoading)
  const error = useSelector(selectFavoritesError)

  const favoriteProducts = useMemo(() => {
    return (rawFavoriteProducts || [])
  ...
  if (loading && favoriteProducts.length === 0) {
    return (
      <>
        ...
        <main
          className="min-h-screen text-purple-400 flex items-center justify-center pt-24"
        ...
        >
          <div className="text-center">
            ...
            <p className="text-lg font-semibold mb-2">
              Loading favorites from database...
            </p>

            <p className="text-sm text-purple-400/60">
              Fetching your saved products from MongoDB
            </p>
          </div>
        </main>
      </>
  ...
  return (
    <>
      ...
      <main
        className="min-h-screen text-purple-400 pt-24"
        style={{ background: '#09070f' }}
      >
        <style>{`
          @import url('https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700;800&display=swap');
            ...
            <div>
              ...
              <p className="mt-3 text-purple-100/60 text-sm">
                {currentUser?.fullName
                  ? `${currentUser.fullName}'s saved products`
          ...
          {favoriteProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
              ...
              <div>
                ...
                <p className="text-purple-100/60 text-sm max-w-md leading-relaxed">
                  Browse our products and click the heart icon on any item to
                  save it here.
                </p>
              </div>
